...
- **Main repo commit:** d3bf25ef (HEAD of verity main)
- **Merged at:** 2026-06-07

PRs since previous checkpoint (#509):

- #510 feat(compliance): per-domain readiness threshold slider UI — Ships the UI for per-domain configurable thresholds (deferred from the scoring refactor). Inline slider on domain detail page (admin/owner only), null state "No target set" + "Set target" CTA initializing to 80%, optimistic UI with 400ms debounced PATCH and AbortController, fire-on-blur, server-confirmed value. Domain progress bar adopts threshold-relative semantics (fills to 100% at threshold, 3-band color, amber ▲ when below). New `Slider` UI primitive. 12 vitest cases.

### Impact

- **walkthrough.mdx**: Added "Readiness threshold slider" subsection in the domain detail section describing the admin-only threshold slider, null state UX, debounced save, and threshold-relative progress bar with 3-band color scheme.
- **features/scoring.mdx**: Expanded the per-domain configurable thresholds paragraph with slider null-state UX details ("No target set" / "Set target" at 80%), optimistic UI, and progress bar 3-band color scheme.

### Build verification

- `next build` passed — 33/33 static pages generated, no errors.
...
Major update — scoring.mdx, roadmap.mdx, walkthrough.mdx, index.mdx, todo.mdx, architecture.mdx, data-model.mdx, background-jobs.mdx, evidence.mdx, dashboard.mdx all updated for scoring engine v2 and v1 historical series.
